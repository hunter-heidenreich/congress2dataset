...
async def scrape(
    congress: int, sleep: int, timeout: int = 60, logger: logging.Logger = None
):
    async with async_playwright() as p:
        ...
        for bill_type in [
            ...
            logger.info(
                f"Scraping {congress}th congress {bill_type} bill texts"
            ) if logger else None
            i = 1
            run = True
            while run:
...
if __name__ == "__main__":
    ...
    handlers = [
        logging.FileHandler(
            f"logs/scrape-bill_text-{datetime.now().strftime('%Y%m%d%H%M%S')}.log"
        ),
