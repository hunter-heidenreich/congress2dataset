...
def read_local_db(congress: int, bill_type: str, i: int, bill_collection):
    try:
        return bill_collection.find_one({"congress": congress, "type": bill_type, "number": i})
...
def parse_overview(bill: dict, bill_soup: BeautifulSoup, logger: logging.Logger = None):
    ...
    overview = bill_soup.find('div', class_='overview_wrapper bill').find(
        'div', class_='overview').find('table')
    out = dict()
    for row in overview.find_all('tr'):
        ...
        if key == 'sponsor':
        ...
        elif key in {'committees', 'latest_action', 'latest_action_(modified)', 'roll_call_votes', 'committee_meetings', 'committee_prints', 'notes'}:
            continue
            ...
            for li in li_elements:
                ...
                if div:
                    ...
                    step_info['actionDate'] = datetime.strptime(step_info['actionDate'], '%Y-%m-%d')
            ...
            out['reports'] = [
                {
                    'url': 'https://www.congress.gov' + a['href'],
                    'title': a.text
                }
        ...
        if head == 'More on This Bill':
            ...
            for li in lis:
                ...
                if a['id'] == 'constAuthButton':
                    bill['constitutional_authority_statement'] = parse_authority_statement(li)
...
def parse_titles(bill: dict, bill_soup: BeautifulSoup, logger: logging.Logger = None):
    titles_content = bill_soup.find('div', id='titles-content').find('div', id='titles_main')
    ...
    if len(child_divs) == 3:
        ...
        assert all([a == b for a, b in zip(child_div_classes, child_div_expected)]), f"Invalid div classes: {child_div_classes}"
    ...
    for _titles in all_titles:
        for chamber in ['house', 'senate']:
            ...
            if _sel:
                for h4 in _sel.find_all('h4'):
                    ...
                    if neighbor.name == 'p': 
                        ...
                        for txt in txts:
                            titles.append({
                                'scope': 'full',
                                'type': 'official' if 'Official' in h4.text else 'short',
                                'chamber': chamber,
                                'title': txt,
                                'label': h4.text
                            })
                ...
                for h5 in _sel.find_all('h5'):
                    ...
                    if neighbor.name == 'ul':
                        for li in neighbor.find_all('li'):
                            titles.append({
                                'scope': 'partial',
                                'type': 'official' if 'Official' in h5.text else 'short',
                                'chamber': chamber,
                                'title': li.text,
                                'label': h5.text
                            })
                ...
                logger.warning(f"Chamber titles column not found: {chamber}") if logger else None
...
def parse(congress: int, logger: logging.Logger = None):
    ...
    for bill_type in [
        'house-bill', 
        'house-resolution', 'house-concurrent-resolution', 'house-joint-resolution',
        'senate-bill', 'senate-resolution', 'senate-concurrent-resolution', 'senate-joint-resolution'
    ]:
        logger.info(f"Parsing {congress}th congress {bill_type} bills") if logger else None
        i = 1
        run = True
        while run:
            logger.info(f"Parsing {congress}th congress {bill_type} bill {i}") if logger else None

            try:
                bill_soup = read_local_html(congress, bill_type, i)
            except FileNotFoundError as e:
                run = input(f"Error: {str(e)}. Retry? (y/n) ") == "y"
                print()
                continue
            
            if not bill_soup.title.text.startswith('All Info - '):
                logger.error(f"Invalid page title: {bill_soup.title.text}") if logger else None
                i += 1
                continue
            ...
            logger.info(f"Parsed {congress}th congress {bill_type} bill {i}") if logger else None
...
if __name__ == "__main__":
    ...
    handlers = [
        logging.FileHandler(f"logs/parse-bill-{datetime.now().strftime('%Y%m%d%H%M%S')}.log"),
